...
const loadstudentform = async (req,res)=>{
    let sql = `select languagename from alllanguages`;
    let result = await con.query(sql);
    let languages = result[0]
    res.render("ajax/studentform",{languages});
}
...
const savestudentdetails = async (req,res)=>{
    ...
    let values = [data.firstname,data.lastname,data.post,data.address1,data.address2,data.email,data.city,data.phone,data.state,data.gender,data.zipcode,data.rs,data.dob];
    ...
    else
    {
        let sql = "INSERT INTO educational_details(id,ssc_board,ssc_year,ssc_percentage,hsc_board,hsc_year,hsc_percentage,bachelor_degree,bachelor_university,bachelor_year,bachelor_percentage,master_degree,master_university,master_year,master_percentage) VALUES(?)";
            ...
            let result = await con.query(sql,[values]);
                ...
                let sql1 = `select languagename from alllanguages`;
                let result1 = await con.query(sql1);
                let languages = result1[0]
                res.render("ajax/studentform",{languages});
}
...
const setstudentdetails = async (req,res)=>{
    let id = req.query.Id;
...
const reinsert = async (req,res)=>{
    let id = req.query.Id;
    let data = req.body;
    let bd = {"firstname":data.firstname,"lastname":data.lastname,"post":data.post,"address1":data.address1,"address2":data.address2,"email":data.email,"city":data.city,"phone":data.phone,"state":data.state,"gender":data.gender,"zipcode":data.zipcode,"relationship":data.rs,"dob":data.dob}
    ...
    akt = await checktechnology(id);
                ...
                {
                    for(let i=0;i<data.contact_name.length;i++)
                {
...
module.exports = {loadstudentform,savestudentdetails,studentlist,updatestudentform,setstudentdetails,reinsert};